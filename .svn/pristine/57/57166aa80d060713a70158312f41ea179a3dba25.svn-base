...
public class InspectionService {
    ...
    public func saveInspectionReportToCoreData(inspection : Inspection) {
        ...
        inspectionEntity = NSEntityDescription.insertNewObjectForEntityForName("Inspection", inManagedObjectContext: managedObjectContext)  as! CdInspection
        ...
        if error != nil {
            ...
            if retry < 2 {
                saveInspectionReportToCoreData(inspection)
                retry++
            }
        } else {
            InspectionBackgroundService.unpostedInspections.append(self.inspectionEntity.documentNumber)
        }
    }
    ...
    private func assignSeals(seals: [Inspection.SealData]) {
        ...
        for singleSeal in seals {
            ...
            if let sealImages = singleSeal.sealImages {
                for image in sealImages {
                    let sealImage: CdSealImage = NSEntityDescription.insertNewObjectForEntityForName("SealImage", inManagedObjectContext: managedObjectContext)  as! CdSealImage
                    
                    sealImage.data = UIImageJPEGRepresentation(image, 1)
                    sealImage.identity = NSUUID().UUIDString
                    
                    sealImage.sealData = sealEntity
                    sealEntity.images.addObject(sealImage)
                }
    ...
    private func assignTireData(tires: [Inspection.TireData]) {
        ...
        for (index, singleTire) in enumerate(tires) {
            ...
            if let psiHose = singleTire.psiHose {
                var triStateData = assignSingleTriStateData("Tire #\(index) Psi Hose", triStateDate: psiHose)
                
                tireEntity.psiHose = triStateData
            }
    ...
    #if DEBUG
    public func printInspection() {
        
        var error: NSError? = nil
        var fetchRequest: NSFetchRequest = NSFetchRequest(entityName: "Inspection")
        fetchRequest.returnsObjectsAsFaults = false
        
        var inspections = managedObjectContext.executeFetchRequest(fetchRequest, error: &error)
        for (index, inspection) in enumerate(inspections!)  {
            
            var item = inspection as! CdInspection
            WernerLog("Inspection #\(index)= \(item.description)")
        }
    }
    
    public func printInspectionDamagedImages() {
        
        var error: NSError? = nil
        var fetchRequest: NSFetchRequest = NSFetchRequest(entityName: "DamagedImage")
        fetchRequest.returnsObjectsAsFaults = false
        
        var images = managedObjectContext.executeFetchRequest(fetchRequest, error: &error)
        for (index, image) in enumerate(images!)  {
            
            var item = image as! CdDamagedImage
            WernerLog("Image #\(index)= \(item.description)")
        }
    }
